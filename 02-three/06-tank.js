...
export default function scene06 (app) {
  ...
  app.updateCamera = function (width, height) {
    cameras.forEach(({ cam }) => {
  ...
  app.animate(time => {
    ...
    // switch camera overtime
    const camera = cameras[time * .25 % cameras.length | 0];
    infoElem.textContent = camera.desc;
    app.camera = camera.cam
  })
}
